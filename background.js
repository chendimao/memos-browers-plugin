...
const hasContextMenus = Boolean(contextMenusApi?.create && contextMenusApi?.onClicked)
const hasPopupApi = Boolean(extensionApi.action?.openPopup)
...
const openPopup = async () => {
  if (!hasPopupApi) {
    console.warn('Memos: 当前浏览器不支持 action.openPopup，请手动点击扩展图标打开弹窗')
    return
  }
...
extensionApi.runtime.onInstalled.addListener(async () => {
  if (!hasContextMenus) {
    console.warn('Memos: 当前浏览器不支持 contextMenus，跳过右键菜单注册')
    return
  }
